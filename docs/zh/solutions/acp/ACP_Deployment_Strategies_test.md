...
该过程涉及两个相同的应用程序环境——`blue` 用于当前生产版本，`green` 用于即将发布的新版本。本文档将逐步介绍以下操作步骤：
...
首先创建您应用程序的稳定版本。这是用户将访问的初始版本。以下是具有适当副本数、容器镜像版本（例如，`hello:1.23.1`）和适当标签（如 `app=web-stable`）的部署。
...
最初，服务选择器针对标记为 `app=web` 和 `version=stable` 的 Pod。
